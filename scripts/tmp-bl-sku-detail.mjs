...
const sku = process.argv[3] || 'ATAS0083';
const rows = parseCsv(readFileSync(csvPath, 'utf8'));
const r = rows.find((x) => x.produkt_sku === sku);
if (!r) { console.log('not found'); process.exit(0); }
console.log('sku', sku, 'id', r.produkt_id);
console.log('zdjecie', r.zdjecie);
for (let i = 1; i <= 5; i++) {
  const u = r[`zdjecie_dodatkowe_${i}`];
  if (u) console.log('extra', i, u);
}
