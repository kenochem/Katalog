...
function New-WaproSkeletonProduct($row) {
  ...
  return [ordered]@{
    ...
    description = ''
    has_image = $false
    stock = (Get-JsonNumberOrNull $row 'stock')
    stock_manual = $false
...
function Invoke-WaproMagAutoImport {
  ...
  try {
    ...
    foreach ($row in $rows) {
      $sku = (Get-JsonTextValue $row 'sku').ToUpperInvariant()
      if (-not $sku -or $sku.StartsWith('X')) { continue }
    ...
    for ($i = 0; $i -lt $candidates.Count; $i += $batchSize) {
      ...
      $body = $chunk | ConvertTo-Json -Depth 8 -Compress
      $uri = '{0}/rest/v1/products' -f $SupabaseUrl
      Invoke-RestMethod -Uri $uri -Headers $jsonHeaders -Method Post -Body $body | Out-Null
