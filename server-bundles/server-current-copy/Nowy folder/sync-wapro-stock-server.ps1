...
function Invoke-WaproMagAutoImport {
  ...
  $script = Join-Path $SyncDirPath 'wapro-auto-import.mjs'
  if (-not (Test-Path $catalog)) {
    return @{ inserted = 0; note = 'brak wapro-mag-catalog.json' }
  }
  if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    return @{ inserted = 0; note = 'brak node.exe — zainstaluj Node LTS na serwerze sync' }
  }
  if (-not (Test-Path $script)) {
    return @{ inserted = 0; note = 'brak wapro-auto-import.mjs (install-wapro-sync-bundle.ps1 z repo)' }
  }
  $prevUrl = $env:SUPABASE_URL
  $prevKey = $env:SUPABASE_SERVICE_ROLE_KEY
  $prevCat = $env:WAPRO_MAG_CATALOG
  try {
    $env:SUPABASE_URL = $SupabaseUrl
    $env:SUPABASE_SERVICE_ROLE_KEY = $ServiceKey
    $env:WAPRO_MAG_CATALOG = $catalog
    $out = & node $script 2>&1 | Out-String
    $trim = $out.Trim()
    if ($trim) { Write-Log ("WAPRO auto-import: {0}" -f ($trim -replace '\s+', ' ')) }
    if ($trim -match '"inserted"\s*:\s*(\d+)') {
      return @{ inserted = [int]$Matches[1]; note = 'ok' }
    }
    return @{ inserted = 0; note = 'brak dopasowania inserted w odpowiedzi' }
  } catch {
    ...
    $env:SUPABASE_SERVICE_ROLE_KEY = $prevKey
    $env:WAPRO_MAG_CATALOG = $prevCat
  }
}
...
function Test-WaproMapHasPrices([hashtable]$stockBySku, [int]$sampleSize) {
  ...
  foreach ($sku in $stockBySku.Keys) {
    $r = $stockBySku[$sku]
    if (($null -ne $r.price_purchase_net) -or ($null -ne $r.price_sale_net) -or ($null -ne $r.price_sale_gross)) {
      return $true
    }
    $n++
...
'@
  ...
  foreach ($pq in $priceQueryVariants) {
    ...
    if (Test-SqlExportError $pCode $pRaw) {
      Write-Log ('SQL ceny wariant {0} nieudany (kod {1})' -f $priceVariantIdx, $pCode)
      continue
    }
  ...
  do {
    if ($syncScope -eq 'accessories' -or $syncScope -eq 'shop') {
      $uri = '{0}/rest/v1/products?catalog=eq.{1}&select=id,sku,stock,stock_manual,is_group,variants,price_purchase_net,price_sale_net,price_sale_gross,catalog,product_meta&offset={2}&limit={3}&order=id' -f $SupabaseUrl, $syncScope, $from, $page
    } else {
      $uri = '{0}/rest/v1/products?select=id,sku,stock,stock_manual,is_group,variants,price_purchase_net,price_sale_net,price_sale_gross,catalog,product_meta&offset={1}&limit={2}&order=id' -f $SupabaseUrl, $from, $page
    }
    $batch = Invoke-RestMethod -Uri $uri -Headers $headers -Method Get
    if ($null -eq $batch) { break }
    $arr = @($batch)
  ...
  $msg = 'Tryb WAPRO: {0}. Zakres: {1}. Zaktualizowano: {2}, bez zmian: {3}, reczne stan: {4}, brak w WAPRO: {5}, uzupelnione pola cen: {6}, SKU z SQL: {7}, dopasowane po legacy/alt SKU: {8}, odkryte (pierwsze stany/ceny): {9}, ostrzezenia: {10}' -f `
    $waproPriceMode, $syncScope, $updated, $unchanged, $skippedManual, $skippedMissing, $pricesFilled, $stockBySku.Count, $linkedViaAlt, $bootstrapped, $warnings
  ...
  if ($salesSyncPendingIds.Count -gt 0) {
    try {
      Invoke-SyncWaproSalesBulk -SupabaseUrl $SupabaseUrl -Headers $headers -JsonHeaders $jsonHeaders -SyncDirPath $SyncDir -PendingIds $salesSyncPendingIds | Out-Null
    } catch {
